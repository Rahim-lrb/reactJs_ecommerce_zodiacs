...
export default function Home() {
  ...
  // const { data: categoriesData } = useFetch('https://fakestoreapi.com/products/categories',{ method: 'GET' });
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const { data } = await axios.get(`https://api.escuelajs.co/api/v1/products?${searchQuery}`);
      
      setDisplayedData(data);
      // setDataEnd(data.length < 12);
      setIsLoading(false)
    } catch (error) {
  ...
  const handleLoadMore = () => {
    setSearchParams(prevSearchParams => ({
      ...prevSearchParams,
      offset: prevSearchParams.offset + 12
    }));
  };
  ...
  return (
    <div className="flex flex-col justify-center">
            ...
            <div className="flex items-center relative">
              ...
              {isOpen && (
                <div className="absolute right-10 top-4 mt-2 w-48 bg-white border rounded-md shadow-lg z-50">
                    <p onClick={() => { setIsOpen(false); setSearchParams(prevState => ({ ...prevState, categoryId: null })) }} className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 cursor-pointer">all</p>

                    {categoriesData.map((cat) => (
                    <p key={cat.id} onClick={() => {setIsOpen(false); setSearchParams(prevState => ({...prevState, categoryId: cat.id})) } } className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 cursor-pointer">{cat.name}</p>
                  ))}
                </div>
              )}
      ...
      <h2 onClick={handleLoadMore}>load more</h2>
    </div>
  );
}
